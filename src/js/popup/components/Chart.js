...
export default class Chart extends Component {
	render() {
		...
		for (let i = 0; i < dateFilteredPrices.length; i++) {
			let obj = {};
			const formattedDate = new Date(dateFilteredPrices[i].date).toUTCString().slice(0, 16);
			obj.x = formattedDate;
			...
			if (i === 0 && formattedDate !== weekAxis[0]) {
				...
				const addingPrice = historicalPricesArr[dateModifiedPricesArr.indexOf(formattedDate) - 1];
		...
		// for sentiment data-----------------------------------
		const dateFilteredSentiment = this.props.historicalArticlesArr.filter((elem) =>
			weekAxis.includes(new Date(elem.date).toUTCString().slice(0, 16))
		);
		...
		for (let i = 0; i < dateFilteredSentiment.length; i++) {
			let obj = {};
			const formattedDate = new Date(dateFilteredSentiment[i].date).toUTCString().slice(0, 16);
			obj.x = formattedDate;
